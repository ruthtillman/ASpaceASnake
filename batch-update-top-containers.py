...
# This file performs batch updates to top container records, adding locations. The API expects to add a single location to every top container which is in that location. It expects a two-column CSV with columns 'location_uri' and 'id'. 'id' is pipe-separated ("|") list with all the IDs of top containers in that location.
# Sample data:
# location_uri,id
# /locations/6423,6631
